    ...
    let chrome_path = find_chrome_via_registry()
        ...
        .ok_or_else(|| {
            io::Error::new(
                ...
                "Chrome executable not found in registry or PATH",
            )
        })?;
...
pub async fn start_gui() -> eframe::Result<()> {
    ...
    let git_sha = env!("GIT_SHORT_SHA");
    let ret = eframe::run_native(
        //"DebugChrome GUI" ,//
        &format!("debugchrome:\\ {} [{}]", env!("CARGO_PKG_VERSION"), git_sha),
        native_options,
        Box::new(|cc| Ok(Box::new(DemoApp::new(cc)))),
    );
    ret.as_ref().unwrap_or_else(|err| {
        eprintln!("Application exited with error: {:?}", err);
        log::error!("Application exited with error: {:?}", err);
        std::process::exit(1);
    });
    ret
}
